...
def update_account_status(user_id: int, account_status_data: UserUpdateAccountStatusRequest, db: Session = Depends(get_db)):
    return user_service.update_account_status(db, user_id, account_status_data)

@router.patch("/{user_id}/current_partner_requests", response_model=UserResponse)
def update_current_partner_requests(user_id: int, current_partner_requests_data: UserUpdateCurrentPartnerRequestsRequest, db: Session = Depends(get_db)):
